...
const getAppHtml = (store, location, universalCookies, context = {}) => {
...
const renderPage = (req, res, store, themeText, placeholders) => {
	const appHtml = getAppHtml(store, req.url, req.universalCookies);
	...
	const html = indexHtml
		...
		.replace('{language}', req.params.locale)
	...
	loadState(req)
		.then(({ state, themeText, placeholders }) => {
			initOnServer({
				...
				tenantUrlName: req.params.tenantUrlName
			});
