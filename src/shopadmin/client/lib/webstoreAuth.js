...
const LOGIN_PATH = `/${tenantUrlName}/${locale}/admin/apps/login`;
const HOME_PATH = `/${tenantUrlName}/${locale}/admin/apps`;
		...
		if (token && token !== '') {
			...
			if (tokenData) {
				...
				if (expiration_date > Date.now()) {
					saveLoginItems({ token, email: tokenData.email, expiration_date });
