...
import adminSetting from 'lib/settings';
...
class FileItem extends React.Component {
	...
	render() {
		...
		const tenantId = utils.decrypt(localStorage.getItem('tenant_id'));
		const fileUrl = `${settings.domain}/${tenantId}/${
			adminSetting.filesUploadUrl
		}/${file.file}`;
		...
		return (
			<div className={style.item + ' row row--no-gutter middle-xs'}>
				...
				<div className={style.more + ' col-xs-2'}>
					{userScopes.includes(scopes.DELETE_FILE) && (
						<Fragment>
	...
	render() {
		...
		return (
			<Fragment>
				...
				{userScopes.includes(scopes.WRITE_FILE) && (
