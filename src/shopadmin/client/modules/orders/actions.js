...
const getFilter = (state, offset = 0) => {
	...
	if (filterState.cart !== null) {
		filter.cart = filterState.cart;
	}

	return filter;
};

function fetchOrdersBySeller(dispatch, filter, state) {
	const seller = state.orders.filter.seller;
	return api.orders
		.list(filter)
		.then(({ status, json }) => {
			const orders = {};
...
export function fetchOrders() {
	return (dispatch, getState) => {
		...
		if (!state.orders.loadingItems) {
			...
			return fetchOrdersBySeller(dispatch, filter, state);
		}
	};
}
...
export function fetchOrder(orderId) {
	return (dispatch, getState) => {
		...
		return api.orders
			.retrieve(orderId)
			.then(orderResponse =>
				helper.filterItemsBySeller(orderResponse.json, seller)
			)
	...
	return (dispatch, getState) => {
		...
		return api.orders
			...
			.then(orderResponse => {
				...
				history.push(`/${tenantUrlName}/${locale}/admin/orders/${orderId}`);
			})
			.catch(error => {});
	};
}
