...
function requestSellers() {
	return {
		type: t.SELLERS_REQUEST
	};
}

function receiveSellers({ total_count, data }) {
	return {
		type: t.SELLERS_RECEIVE,
		total_count,
		data
	};
}

function receiveSellersError(error) {
	return {
		type: t.SELLERS_FAILURE,
		error
	};
}
...
const getFilter = (state, offset = 0) => {
	...
	const seller = state.products.filter.seller;

	let filter = {
		limit: 30,
		fields:
			'id,name,category_id,category_ids,category_name,sku,images,enabled,discontinued,stock_status,stock_quantity,price,on_sale,regular_price,url,seller',
		search: searchTerm,
		offset: offset,
		sort: sortOrder,
		seller: seller
	...
	return (dispatch, getState) => {
		...
		return api.products
			...
			.then(({ status, json }) => {
				...
				history.push(`/${tenantUrlName}/${locale}/admin/products/` + json.id);
			})
			.catch(error => {});
	};
}
