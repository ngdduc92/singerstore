...
export default class ProductImages extends React.Component {
	...
	render() {
		...
		const allowEdit = userScopes.includes(scopes.WRITE_PRODUCT);
		return (
			<Paper className="paper-box" zDepth={1}>
				<div style={{ padding: '10px 10px 30px 10px' }}>
					<Gallery
						productId={productId}
						images={images}
						onImageDelete={allowEdit ? onImageDelete : () => {}}
						onImageSort={allowEdit ? onImageSort : () => {}}
						onImageUpload={allowEdit ? onImageUpload : () => {}}
						uploading={allowEdit ? uploadingImages : false}
						onImageEdit={allowEdit ? this.handleEditOpen : () => {}}
					/>
					<Dialog
