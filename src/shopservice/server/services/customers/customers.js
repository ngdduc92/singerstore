...
class CustomersService {
	...
	getSingleCustomer(tenantId, id) {
		if (!ObjectID.isValid(id)) {
			return Promise.reject('Invalid identifier');
		}
		return this.getCustomers(tenantId, { id: id }).then(items =>
			items.data.length > 0 ? items.data[0] : {}
		);
	}

	async addCustomer(tenantId, data) {
		const customer = this.getValidDocumentForInsert(tenantId, data);
		...
		const newCustomer = await this.getSingleCustomer(tenantId, newCustomerId);
	...
	async updateCustomer(tenantId, id, data) {
		...
		const updatedCustomer = await this.getSingleCustomer(tenantId, id);
		await webhooks.trigger({
	...
	deleteCustomer(tenantId, customerId) {
		...
		const customerObjectID = new ObjectID(customerId);
		return this.getSingleCustomer(tenantId, customerId).then(customer => {
			webhooks.trigger({
			...
			return db
				.collection('customers')
				.updateOne({ _id: customerObjectID }, { $set: deletedData })
	...
	getValidDocumentForInsert(tenantId, data) {
		...
		customer.tenant_id = parse.getObjectIDIfValid(tenantId);
		customer.deleted = parse.getBooleanIfValid(data.deleted, false);
