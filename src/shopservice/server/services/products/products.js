...
class ProductsService {
	...
	async getProducts(tenantId, params = {}) {
		...
		const matchQuery = this.getMatchQuery(tenantId, params, categories);
		...
		let items = itemsResult.map(item =>
			this.changeProperties(tenantId, item, domain)
		);
	...
	getMatchQuery(
		...
		// parse values
		tenantId = parse.getObjectIDIfValid(tenantId);
	...
	getSingleProduct(tenantId, id) {
		if (!ObjectID.isValid(id)) {
			return Promise.reject('Invalid identifier');
		}
		return this.getProducts(tenantId, { ids: id, limit: 1 }).then(products =>
			products.data.length > 0 ? products.data[0] : {}
		);
	}

	addProduct(tenantId, data) {
		return this.getValidDocumentForInsert(tenantId, data)
			.then(dataToInsert =>
				db.collection('products').insertMany([dataToInsert])
			)
			.then(res => this.getSingleProduct(tenantId, res.ops[0]._id.toString()));
	}
	...
	updateProduct(tenantId, id, data) {
		...
		return this.getValidDocumentForUpdate(id, data)
			...
			.then(res =>
				res.modifiedCount > 0 ? this.getSingleProduct(tenantId, id) : null
			);
	}
	...
	deleteProduct(productId) {
		...
		return db
			.collection('products')
			.updateOne({ _id: productObjectID }, { $set: deletedData })
	...
	getValidDocumentForInsert(tenantId, data) {
		...
		product.tenant_id = parse.getObjectIDIfValid(tenantId);
		product.deleted = parse.getBooleanIfValid(data.deleted, false);
		product.seller = parse.getString(data.seller);
	...
	getSortedImagesWithUrls(tenantId, item, domain) {
	...
	getImageUrl(tenantId, domain, productId, imageFileName) {
		return url.resolve(
			domain,
			`/${tenantId}/${settings.productsUploadUrl}/${productId}/${imageFileName}`
		);
	}
	...
	changeProperties(tenantId, item, domain) {
		if (item) {
			...
			item.images = this.getSortedImagesWithUrls(tenantId, item, domain);
