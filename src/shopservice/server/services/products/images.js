...
class ProductImagesService {
	...
	getImages(tenantId, productId) {
		if (!ObjectID.isValid(productId)) {
			return Promise.reject('Invalid identifier');
		}
		let productObjectID = new ObjectID(productId);

		return SettingsService.getSettings(tenantId).then(generalSettings =>
			db
				.collection('products')
					...
					if (product && product.images && product.images.length > 0) {
						let images = product.images.map(image => {
							image.url = url.resolve(
								...
								`/${tenantId}/${settings.productsUploadUrl}/${product._id}/${
									image.filename
								}`
							);
							return image;
	...
	deleteImage(tenantId, productId, imageId) {
		...
		return this.getImages(tenantId, productId)
			.then(images => {
	...
	async addImage(req, res) {
		...
		const uploadDir = path.resolve(
			`${settings.uploadPath}/${req.get('x-tenant-id')}/${
				settings.productsUploadPath
			}/${productId}`
		);
