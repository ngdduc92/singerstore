...
class OrderItemsService {
	...
	async addItem(tenantId, order_id, data) {
		...
		if (orderItem) {
			...
			await this.addNewItem(tenantId, order_id, newItem);
		}

		return OrdersService.getSingleOrder(tenantId, order_id);
	}

	async updateItemQuantityIfAvailable(tenantId, order_id, orderItem, newItem) {
		...
		if (availableQuantity > 0) {
			await this.updateItem(tenantId, order_id, orderItem.id, {
				quantity: availableQuantity
			});
		}
	}

	async addNewItem(tenantId, order_id, newItem) {
		...
		if (availableQuantity > 0) {
			...
			await this.calculateAndUpdateItem(tenantId, order_id, newItem.id);
			await ProductStockService.handleAddOrderItem(
				tenantId,
				order_id,
				newItem.id
			);
	...
	async updateItem(tenantId, order_id, item_id, data) {
		...
		if (parse.getNumberIfPositive(data.quantity) === 0) {
			// delete item
			return this.deleteItem(tenantId, order_id, item_id);
		} else {
			// update
			await ProductStockService.handleDeleteOrderItem(
				tenantId,
				order_id,
				item_id
			);
			...
			await this.calculateAndUpdateItem(tenantId, order_id, item_id);
			await ProductStockService.handleAddOrderItem(tenantId, order_id, item_id);
			return OrdersService.getSingleOrder(tenantId, order_id);
		}
	}
	...
	async calculateAndUpdateItem(tenantId, orderId, itemId) {
		...
		if (order && order.items && order.items.length > 0) {
			...
			if (item) {
				const itemData = await this.getCalculatedData(tenantId, item);
				await db.collection('orders').updateOne(
					{
						_id: orderObjectID,
						'items.id': itemObjectID
	...
	async getCalculatedData(tenantId, item) {
		const product = await ProductsService.getSingleProduct(
			tenantId,
			item.product_id.toString()
		);

		const categories = this.getCategoriesFromProduct(product);
		...
		if (item.custom_price && item.custom_price > 0) {
			...
			return {
				...
				'items.$.category_ids': categories,
				'items.$.seller': product.seller
			};
	...
	async calculateAndUpdateAllItems(tenantId, order_id) {
		const order = await OrdersService.getSingleOrder(tenantId, order_id);

		if (order && order.items) {
			for (const item of order.items) {
				await this.calculateAndUpdateItem(tenantId, order_id, item.id);
			}
			return OrdersService.getSingleOrder(tenantId, order_id);
		} else {
	...
	async deleteItem(tenantId, order_id, item_id) {
		if (!ObjectID.isValid(order_id) || !ObjectID.isValid(item_id)) {
			return Promise.reject('Invalid identifier');
		}
		let orderObjectID = new ObjectID(order_id);
	...
	getCategoriesFromProduct(product) {
		const categories = [];
		categories.push(parse.getObjectIDIfValid(product.category_id));
		if (product.category_ids && product.category_ids.length > 0) {
			product.category_ids.map(id =>
				categories.push(parse.getObjectIDIfValid(id))
			);
		}
		return parse.getArrayOfObjectID(categories);
	}
}
