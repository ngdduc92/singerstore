...
class OrdersService {
	...
	getFilter(tenantId, params = {}) {
		...
		const cart = parse.getBooleanIfValid(params.cart);
	...
	getOrders(tenantId, params) {
		let filter = this.getFilter(tenantId, params);
		...
		return Promise.all([
			...
			OrderStatusesService.getStatuses(tenantId),
	...
	getSingleOrder(tenantId, id) {
		if (!ObjectID.isValid(id)) {
			return Promise.reject('Invalid identifier');
		}
		return this.getOrders(tenantId, { id: id }).then(items =>
			items.data.length > 0 ? items.data[0] : {}
		);
	}

	getOrCreateCustomer(tenantId, orderId) {
		return this.getSingleOrder(tenantId, orderId).then(order => {
			if (!order.customer_id && order.email) {
				// find customer by email
				return CustomersService.getCustomers(tenantId, {
					email: order.email
				}).then(customers => {
					...
					if (customerExists) {
						...
						return CustomersService.addCustomer(tenantId, {
							email: order.email,
	...
	async addOrder(tenantId, data) {
		const order = await this.getValidDocumentForInsert(tenantId, data);
		const insertResponse = await db.collection('orders').insertMany([order]);
		const newOrderId = insertResponse.ops[0]._id.toString();
		const newOrder = await this.getSingleOrder(tenantId, newOrderId);
		return newOrder;
	}
	...
	async updateOrder(tenantId, id, data) {
		...
		const updatedOrder = await this.getSingleOrder(tenantId, id);
		...
		await this.updateCustomerStatistics(tenantId, updatedOrder.customer_id);
		return updatedOrder;
	}

	deleteOrder(tenantId, orderId) {
		...
		return this.getSingleOrder(tenantId, orderId).then(order => {
			...
			return db
				.collection('orders')
				.updateOne({ _id: orderObjectID }, { $set: { deleted: true } })
				.then(res => (res.modifiedCount > 0 ? true : null));
		});
	}
	...
	getValidDocumentForInsert(tenantId, data) {
		return db
			...
			.then(items => {
				...
				order.cart = parse.getBooleanIfValid(data.cart, false);
				...
				order.tenant_id = parse.getObjectIDIfValid(tenantId);
				order.deleted = parse.getBooleanIfValid(data.deleted, false);
	...
	getValidDocumentForUpdate(id, data) {
		return new Promise((resolve, reject) => {
			...
			if (data.coupon !== undefined) {
				order.coupon = parse.getCoupon(data.coupon);
			}
	...
	changeProperties(order, orderStatuses, shippingMethods, paymentMethods) {
		if (order) {
			...
			order.tax_included_total = tax_included_total;
	...
	async checkoutOrder(tenantId, orderId) {
		/*
    TODO:
    - check order exists
    ...
    */
		const [order, emailTemplate, dashboardSettings] = await Promise.all([
			this.getOrCreateCustomer(tenantId, orderId).then(customer_id => {
				return this.updateOrder(tenantId, orderId, {
			...
			EmailTemplatesService.getEmailTemplate(tenantId, 'order_confirmation'),
			SettingsService.getSettings(tenantId)
		]);

		const subject = utils.getEmailSubject(emailTemplate, order);
		const body = utils.getEmailBody(emailTemplate, order);
		...
		await Promise.all([
			...
			ProductStockService.handleOrderCheckout(tenantId, orderId)
		]);

		return order;
	}

	cancelOrder(tenantId, orderId) {
		const orderData = {
			cancelled: true,
			date_cancelled: new Date()
		};

		return ProductStockService.handleCancelOrder(tenantId, orderId).then(() =>
			this.updateOrder(tenantId, orderId, orderData)
		);
	}

	closeOrder(tenantId, orderId) {
		const orderData = {
			closed: true,
			date_closed: new Date()
		};

		return this.updateOrder(tenantId, orderId, orderData);
	}

	updateCustomerStatistics(tenantId, customerId) {
		if (customerId) {
			return this.getOrders(tenantId, { customer_id: customerId }).then(
				orders => {
					let totalSpent = 0;
					let ordersCount = 0;
	...
	async chargeOrder(tenantId, orderId) {
		const order = await this.getSingleOrder(tenantId, orderId);
		const isSuccess = await PaymentGateways.processOrderPayment(
			tenantId,
			order
		);
		return isSuccess;
